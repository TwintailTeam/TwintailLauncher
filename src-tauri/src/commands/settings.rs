    ...
    match path_type.as_str() {
        "mods" => {
            ...
            if settings.is_some() {
                ...
                if fp.exists() { app.opener().reveal_item_in_dir(fp.as_path()).unwrap(); } else { send_notification(&app, "XXMI is not downloaded or folder structure is corrupt! Can not open the folder.", None); };
            }
        },
        "install" => {
            ...
            if install.is_some() {
                ...
                if fp.exists() { app.opener().reveal_item_in_dir(fp.as_path()).unwrap(); } else { send_notification(&app, "Can not open game directory, Please run the game once!", None); };
            }
        },
        _ => {}
    }
