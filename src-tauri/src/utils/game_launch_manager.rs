...
pub fn launch(app: &AppHandle, install: LauncherInstall, gm: GameManifest, gs: GlobalSettings) -> Result<bool, Error> {
    ...
    let rslt = if install.launch_command.is_empty() {
        ...
        let mut command = if rm.display_name.to_ascii_lowercase().contains("proton") && !rm.display_name.to_ascii_lowercase().contains("wine (ge") {
        ...
        if install.use_jadeite {
            ...
            command = if rm.display_name.to_ascii_lowercase().contains("proton") && !rm.display_name.to_ascii_lowercase().contains("wine (ge") {
        ...
        if !install.env_vars.is_empty() {
            ...
            splitted.iter().for_each(|env| {
                if !env.is_empty() {
                    let tmp = env.splitn(2,"=").collect::<Vec<&str>>();
                    let k = tmp[0];
                    let v = tmp[1].replace("\"", "");
                    cmd.env(k, v);
                }
