...
pub fn init<R: Runtime>(app: &AppHandle<R>, install: LauncherInstall, gm: GameManifest) -> Option<DiscordIpcClient> {
    ...
    let state = format!("In Game on {platform_string}");

    let payload = activity::Activity::new().activity_display_type(activity::ActivityDisplayType::Details).activity_type(activity::ActivityType::Playing).details(&details).state(&state).timestamps(activity::Timestamps::new().start(start)).assets(activity::Assets::new().large_image(icon_key).large_text(&install.name).small_image("tl_512").small_text(&small_txt));
