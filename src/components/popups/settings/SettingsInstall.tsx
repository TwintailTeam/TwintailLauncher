...
export default function SettingsInstall({setOpenPopup, installSettings, pushInstalls, runnerVersions, dxvkVersions, setCurrentInstall, setCurrentGame, games, setBackground}: {games: any, runnerVersions: any, dxvkVersions: any, installSettings: any, setOpenPopup: (popup: POPUPS) => void, pushInstalls: () => void, setCurrentInstall: (id: string) => void, setCurrentGame: (id: string) => void, setBackground: (id: string) => void}) {
    return (
        <div className="rounded-lg h-full w-3/4 flex flex-col p-4 gap-8 overflow-scroll">
            ...
            <div className="flex flex-row-reverse">
                ...
                <div className={`w-full transition-all duration-500 overflow-hidden bg-neutral-700 gap-4 flex flex-col items-center justify-between px-4 p-4 rounded-b-lg rounded-t-lg`} style={{maxHeight: (20 * 64) + "px"}}>
                    ...
                    <SelectMenu id={"install_fps_value"} name={"FPS value"} options={[{value: "60", name: "60"}]} selected={"60"}/>
